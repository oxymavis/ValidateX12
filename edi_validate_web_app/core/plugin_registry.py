...
from edi_210_validator import validate_edi_210  # type: ignore  # noqa: E402
...
def validate_blujay_210(segments: List[List[str]]) -> List[dict]:
    edi_text = "~".join("*".join(part for part in segment) for segment in segments if segment)
    if edi_text:
        edi_text += "~"
    return validate_edi_210(edi_text)

PLUGIN_REGISTRY: Dict[str, ValidatorFn] = {
    "blujay_210": validate_blujay_210,
