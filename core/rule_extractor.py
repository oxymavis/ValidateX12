...
QUALIFIED_REQUIRED_RE = re.compile(
    r"\b([A-Z]{2,3})\*([A-Z0-9]{2,4})\b.*?(REQUIRED|MANDATORY|MUST USE|必填|必须|缺少)",
    re.IGNORECASE,
)
QUALIFIED_MISSING_RE = re.compile(r"\bMISSING\s+([A-Z]{2,3})\*([A-Z0-9]{2,4})\b", re.IGNORECASE)
ELEMENT_REQUIRED_RE = re.compile(r"\b([A-Z]{2,3}\d{2})\b.*?(REQUIRED|MANDATORY|MUST USE|必填|必须|缺少)", re.IGNORECASE)
SEGMENT_REQUIRED_RE = re.compile(r"\b([A-Z]{2,3})\b(?:\s+SEGMENT)?\s+(?:IS\s+)?(REQUIRED|MANDATORY|MUST USE|必填|必须|缺少)", re.IGNORECASE)
MUST_BE_RE = re.compile(r"\b([A-Z]{2,3}\d{2})\b.*?(?:MUST BE|应为|必须是)\s+([A-Z0-9/\- ,|]+)", re.IGNORECASE)
ONE_OF_RE = re.compile(r"\b([A-Z]{2,3}\d{2})\b.*?(?:ONE OF|其中之一|必须是)\s+([A-Z0-9/\- ,|]+)", re.IGNORECASE)
FORMAT_RE = re.compile(r"\b([A-Z]{2,3}\d{2})\b.*?(CCYYMMDD|YYMMDD|HHMMSS|HHMM)\b", re.IGNORECASE)
CONDITIONAL_RE = re.compile(r"IF\s+([A-Z]{2,3}\d{2})\s+(?:IS\s+)?PRESENT\s+THEN\s+([A-Z]{2,3}\d{2})\s+(?:IS\s+)?REQUIRED", re.IGNORECASE)
PAIRED_RE = re.compile(r"\b([A-Z]{2,3}\d{2})\b\s+AND\s+\b([A-Z]{2,3}\d{2})\b.*?(?:TOGETHER|APPEAR TOGETHER|MUST APPEAR TOGETHER)", re.IGNORECASE)
AT_LEAST_ONE_RE = re.compile(r"AT LEAST ONE OF\s+([A-Z]{2,3}\d{2}(?:\s*(?:,|/|OR)\s*[A-Z]{2,3}\d{2})+)", re.IGNORECASE)
LEVEL_REQUIRED_RE = re.compile(r"\b(SHIPMENT|ORDER|PACK|PACKAGE|TARE|ITEM)\s+(?:LEVEL|HL)?\s+([A-Z]{2,3})\b.*?(REQUIRED|MANDATORY|缺少|必填)", re.IGNORECASE)
LOOP_CONTAINS_RE = re.compile(r"\b(ITEM|PACK|PACKAGE|ORDER|SHIPMENT|TARE)\s+HL\b.*?(?:CONTAIN|WITH)\s+([A-Z]{2,3})(?:\s+AND\s+([A-Z]{2,3}))?", re.IGNORECASE)
